...
export function HabitsView() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto pb-24">
          ...
          <div className="bg-theme-card/50 backdrop-blur-sm border border-theme-border/50 rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              {state.habits.length === 0 && (
                <div className="text-center py-10 text-slate-500 border border-dashed border-theme-border/50 rounded-xl bg-theme-card/30">
                  No habits yet
                  <br />
              ...
              <div className="min-w-fit pr-4">
                <div className="grid grid-flow-col grid-rows-7 gap-1.5" style={{ gridAutoColumns: 'max-content' }}>
                  {heatmapDays.map(day => {
                    ...
                    const intensity = count === 0 ? 'bg-theme-card/80 border-theme-border/30' :
                                     count === 1 ? 'bg-theme-accent/40 border-theme-accent/50' :
                                     count === 2 ? 'bg-theme-accent/70 border-theme-accent/80' :
                                     'bg-theme-accent border-indigo-400';
        ...
        <div>
          <div className="bg-theme-card/50 backdrop-blur-sm border border-theme-border/50 rounded-2xl p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
              ...
              <div className="flex items-center gap-1 bg-theme-card/80 border border-theme-border/50 rounded-lg p-1">
                <button onClick={handlePrevMonth} className="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-theme-border rounded-md transition-colors">
                ...
                <select
                  ...
                  className="bg-transparent text-slate-200 font-medium text-sm outline-none cursor-pointer hover:bg-theme-border rounded-md transition-colors py-1 px-1 appearance-none text-center"
                >
                  {yearOptions.map(y => (
                    <option key={y} value={y} className="bg-theme-card text-slate-200">{y}</option>
                  ))}
                </select>
                <button onClick={handleNextMonth} className="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-theme-border rounded-md transition-colors">
                  ...
                  <button
                    ...
                    className={`aspect-square rounded-xl flex flex-col items-center justify-center relative transition-colors border ${
                      !isCurrentMonth
                        ...
                        : 'text-slate-300 bg-theme-card/40 border-theme-border/30 hover:bg-theme-border hover:border-theme-border'
                    } ${
                      isSelectedDateStr
                        ? 'bg-theme-border border-slate-500 text-white shadow-sm ring-1 ring-slate-500 z-20'
                        : ''
                    } ${
                      isTodayStr && !isSelectedDateStr
                        ? 'bg-theme-accent/20 border-theme-accent/50 text-indigo-300 z-10'
                        : ''
                    }`}
                  >
                    <span className={`text-sm ${isTodayStr || isSelectedDateStr ? 'font-bold' : 'font-medium'} z-10`}>{format(day, 'd')}</span>
                    {hasTask && (
                      <div className={`absolute bottom-1.5 w-1.5 h-1.5 rounded-full ${isTodayStr || isSelectedDateStr ? 'bg-indigo-400' : 'bg-theme-accent/70'}`} />
                    )}
                  </button>
                );
              })}
      ...
      {isFabOpen && (
        <div className="fixed inset-0 bg-theme-bg/80 backdrop-blur-sm z-40 flex flex-col justify-end p-4 pb-32 transition-all" onClick={() => setIsFabOpen(false)}>
          <div
          ...
          >
            <div className="flex justify-between items-center mb-6">
              ...
              <button type="button" onClick={() => setIsFabOpen(false)} className="p-1 text-slate-400 hover:text-slate-200 hover:bg-theme-border rounded-lg transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex-1 cursor-pointer">
                    ...
                    <div className="bg-theme-bg text-slate-400 border border-theme-border/50 rounded-xl font-medium px-4 py-3 text-center peer-checked:bg-theme-accent peer-checked:text-white peer-checked:border-theme-accent transition-colors shadow-sm">
                      Daily
                    </div>
                  </label>
                  <label className="flex-1 cursor-pointer">
      ...
      <button
        ...
        className={`fixed bottom-20 md:bottom-12 right-6 md:right-12 z-50 w-14 h-14 bg-theme-accent text-white rounded-full shadow-lg flex items-center justify-center hover:bg-theme-accent hover:scale-105 active:scale-95 transition-all ${isFabOpen ? 'rotate-45' : ''}`}
      >
...
function HabitRow({ habit }: { habit: Habit }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-theme-card/80 p-3 rounded-xl border border-theme-border/50 group hover:border-theme-border transition-colors gap-3 shadow-sm">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <button
          ...
          className={`w-8 h-8 rounded-lg border flex-shrink-0 flex items-center justify-center transition-colors ${
            isDoneToday
              ...
              : 'bg-theme-bg border-theme-border text-transparent hover:border-theme-accent'
          }`}
        >
        ...
        <div className="flex flex-col min-w-0 truncate pr-2">
          ...
          {isWeekly && (
            <span className="text-[10px] font-semibold text-theme-accent uppercase tracking-wider mt-0.5">
      ...
      <div className="flex items-center gap-3 self-end sm:self-auto pl-11 sm:pl-0 shrink-0">
        ...
        <div className="flex gap-1.5">
          {streak >= 3 && (
            <div title="3 Streak: Hot" className="bg-theme-bg border border-theme-border rounded-md p-1">
              <Flame className="w-4 h-4 text-orange-400 fill-orange-400/20" />
            </div>
          )}
          {isPro && (
            <div title="7 Streak: Pro" className="bg-theme-bg border border-theme-border rounded-md p-1 text-theme-secondary">
              <Target className="w-4 h-4" />
            </div>
          )}
          {isMaster && (
            <div title="30 Streak: Master" className="bg-theme-bg border border-theme-border rounded-md p-1 text-yellow-400 flex items-center justify-center">
        ...
        <div className={`flex items-baseline gap-1 px-2.5 py-1 rounded-md border text-sm font-semibold transition-colors ${
          streak > 0 ? 'bg-theme-secondary/10 border-theme-secondary/20 text-theme-secondary' : 'bg-theme-bg border-theme-border/50 text-slate-500'
        }`}>
        ...
        <button
          onClick={removeHabit}
          className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg text-slate-400 hover:bg-theme-border hover:text-red-400 transition-colors"
        >
